...
def main_input(
        current_frame,
        weights_path=os.path.join(os.path.dirname(os.path.realpath(__file__)), 'weights', 'pretrained_yolo.weights'),
         model_cfg=os.path.join(os.path.dirname(os.path.realpath(__file__)), 'model_cfg', 'yolo_baseline.cfg'),
         conf_thres=0.8,
         nms_thres=0.25,
         xy_loss=2,
         wh_loss=1.6,
...
# NewcastleRacingAI
# Abdullah Turki H Alshadadi
def current_frame_detect(current_frame,model,device,conf_thres,nms_thres):
    ...
    with torch.no_grad():
        ...
        for detections in output:
            ...
            if nms_indices.shape[0] == 0:  
                continue
        frame = cv2.cvtColor(current_frame, cv2.COLOR_BGR2RGB)
        frame_with_boxes = Image.fromarray(frame)
        img_with_boxes = frame_with_boxes.copy()
        ...
        for i in range(len(main_box_corner)):
            ...
            back_to_array = np.asarray(img_with_boxes.copy())
            back_to_frame = cv2.cvtColor(back_to_array, cv2.COLOR_RGB2BGR)

        # Display image
        cv2.imshow("Camera Frame", back_to_frame)
        cv2.waitKey(1)
...
if __name__ == '__main__':
    ...
    #      current_frame=,
    #      weights_path='weights/pretrained_yolo.weights',
