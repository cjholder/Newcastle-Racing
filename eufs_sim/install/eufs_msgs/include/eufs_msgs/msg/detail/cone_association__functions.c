  ...
  eufs_msgs__msg__ConeAssociation__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    eufs_msgs__msg__ConeAssociation * data =
      (eufs_msgs__msg__ConeAssociation *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!eufs_msgs__msg__ConeAssociation__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          eufs_msgs__msg__ConeAssociation__fini(&data[i]);
        }
        free(data);
        return false;
      }
